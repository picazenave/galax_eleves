...
__device__ void bodyBodyInteraction(float4 bi, float4 bj, float4 &ai)
{
	float3 diff;
	diff.x = bj.x - bi.x;
	diff.y = bj.y - bi.y;
...
__device__ void tile_calculation(float4 myPosition, float4 &accel)
{
	int i;
...
__global__ void calculate_forces(float4 *positionsGPU, float4 *accelerationsGPU, int n_particles)
{
